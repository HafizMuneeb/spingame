...
def get_number_of_lines():
    while True:
        lines = input("Enter tyhe number of lines to bet on (1-" + str(MAX_LINES) + ")? ")
        if lines.isdigit():
            lines = int(lines)
...
def spin(balance):
    ...
    while True:
        ...
        if total_bet > balance:
            print(f"You do not have enought balance to bet that amount, your current balance is : ${balance}")
        else:
            break
